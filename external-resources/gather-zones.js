...
const displayPlacesForQuest = (quest) => {
  ...
  quest.conditions.AvailableForFinish.forEach((condition) => {
    ...
    //     if (counterCondition.zoneId) {
    //       res.push({
    //         zoneId: counterCondition.zoneId,
    //         location: MAP_BY_IDS[quest.location],
    //       });
    //     }
    //   });
...
const main = () => {
  ...
  const zones = Object.keys(quests).flatMap((questId) => {
  ...
  zones.forEach((zone) => {
    ...
    if (!resultByMaps[zone.location]) {
      resultByMaps[zone.location] = {};
    }

    resultByMaps[zone.location][zone.zoneId] = true;
  });
